...
ENUM_TIMEFRAMES PeriodoTexto(string txt) {
   string t = txt;
   StringToLower(t);
   // Order matters to avoid collisions (e.g., 15 containing 1 or 5)
   ...
   if(StringFind(t, "h1") >= 0  || StringFind(t, "60 min") >= 0 || StringFind(t, "1 hora") >= 0)  return PERIOD_H1;
   if(StringFind(t, "h4") >= 0  || StringFind(t, "4 horas") >= 0) return PERIOD_H4;
   if(StringFind(t, "d1") >= 0  || StringFind(t, "diário") >= 0)  return PERIOD_D1;
...
void AddRuleSpecific(string seg, ENUM_SIGNAL intent) {
   ...
   if(maPos >= 0 && g_nRules < 40) {
      ...
      string keyword = (StringFind(s, "média", maPos) >= 0) ? "média" : "ma";
      r.p1 = (int)ExtraiNumero(s, keyword, maPos);
      if(r.p1 == 0) r.p1 = 20;

      // Check for second period (MA vs MA)
      int nextPos = StringFind(s, "/", maPos);
      if(nextPos < 0) nextPos = StringFind(s, " e ", maPos);
      if(nextPos >= 0) {
         r.p2 = (int)ExtraiNumero(s, "", nextPos);
      }
...
bool AvaliaRegra(Rule &r) {
   ...
   if(r.type == 1) { // MA
      ...
      if(r.handle2 != INVALID_HANDLE) {
         double ma2_0 = GetVal(r.handle2, 0);
         double ma2_1 = GetVal(r.handle2, 1);
         if(r.op == "cross_above") return (ma1 <= ma2_1 && ma0 > ma2_0);
         if(r.op == "cross_below") return (ma1 >= ma2_1 && ma0 < ma2_0);
         if(r.op == ">") return ma0 > ma2_0;
         if(r.op == "<") return ma0 < ma2_0;
      } else {
   ...
   if(r.type == 9) { // Pattern
      ...
      if(isInside) return (r.intent == SIGNAL_BUY) ? bull : !bull;
      if(isOutside) return (r.intent == SIGNAL_BUY) ? !bull : bull;
   }
...
void EnviaOrdem(string tipo, double preco, double sl, double tp, double lote) {
   // Compatibility wrapper for manual/prompt-driven orders
   if(tipo == "BUY") g_trade.Buy(lote, _Symbol, preco, sl, tp);
   else if(tipo == "SELL") g_trade.Sell(lote, _Symbol, preco, sl, tp);
}

...
void InterpretaPrompt(string prompt) {
   ...
   for(int i=0; i<nParts; i++) {
      ...
      Signal intent = (StringFind(p, "compra") >= 0) ? BUY : (StringFind(p, "vende") >= 0) ? SELL : NONE;
      ...
      if(StringFind(p, "média") >= 0 || StringFind(p, "ma") >= 0) {
         ...
         if(idx == -1) { idx = nRules; nRules++; }

         rules[idx].active = true;
         rules[idx].type = MA_CROSS;
         rules[idx].p1 = per;
         rules[idx].intent = intent;
         if(rules[idx].p1_handle != INVALID_HANDLE) IndicatorRelease(rules[idx].p1_handle);
         rules[idx].p1_handle = iMA(_Symbol, PERIOD_CURRENT, rules[idx].p1, 0, MODE_EMA, PRICE_CLOSE);
      ...
      if(StringFind(p, "rsi") >= 0) {
         ...
         if(rules[idx].p1_handle != INVALID_HANDLE) IndicatorRelease(rules[idx].p1_handle);
         rules[idx].p1_handle = iRSI(_Symbol, PERIOD_CURRENT, rules[idx].p1, PRICE_CLOSE);

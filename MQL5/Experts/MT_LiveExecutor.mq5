...
#define AI_SIGNAL_FILE "signal_ai.txt"
...
struct Rule {
   bool     active;
   int      type;    // 1:MA Cross, 2:RSI, 3:Stoch, 4:BB, 5:DailyBreak, 6:Delta, 7:VolCycle, 8:AMA, 9:Bar2, 10:RelStr
...
int AvaliaRegra(Rule &r) {
   ...
   switch(r.type) {
      case 1: { // MA Cross (Price vs MA or Fast vs Slow)
         ...
         // If f is Fast/Price and s is Slow/MA
      ...
      case 2: { // RSI
         ...
         // Threshold check
         if(StringFind(r.s1, ">") >= 0 || StringFind(r.s1, "acima") >= 0) {
            if(v2 <= r.d1 && v1 > r.d1) return (r.intent == SIGNAL_BUY) ? SIGNAL_BUY : SIGNAL_NONE;
         } else if(StringFind(r.s1, "<") >= 0 || StringFind(r.s1, "abaixo") >= 0) {
            if(v2 >= r.d1 && v1 < r.d1) return (r.intent == SIGNAL_SELL) ? SIGNAL_SELL : SIGNAL_NONE;
         } else {
            // Default oversold/overbought
            if(v1 < 30) return SIGNAL_BUY;
            if(v1 > 70) return SIGNAL_SELL;
         }
         break;
      }
      case 3: { // Stoch Cross
      ...
      case 6: { // Delta Aggression
         ...
         long buy=0, sell=0;
         for(int i=0; i<n; i++) if((ticks[i].flags & TICK_FLAG_BUY) == TICK_FLAG_BUY) buy++; else if((ticks[i].flags & TICK_FLAG_SELL) == TICK_FLAG_SELL) sell++;
         long delta = buy - sell;
      ...
      case 7: { // Volume Cycle
         ...
         if(CopyVolume(_Symbol, (ENUM_TIMEFRAMES)r.tf, 0, r.p1, vol) > 0) {
...
void AvaliaTudo() {
   ...
   int aiSig = AIPredict();
   if(aiSig == SIGNAL_BUY) buyVotos++;
   if(aiSig == SIGNAL_SELL) sellVotos++;
...
void CalculaStats() {
   ...
   for(int i=0; i<total; i++) {
      ...
      if(HistoryDealGetInteger(ticket, DEAL_MAGIC) == EA_MAGIC) {
         ...
         else if(res < 0) { losses++; loss += MathAbs(res); }
      }
   }
...
//+------------------------------------------------------------------+
//| AI Predict Placeholder                                           |
//+------------------------------------------------------------------+
int AIPredict() {
   int h = FileOpen(AI_SIGNAL_FILE, FILE_READ|FILE_TXT|FILE_COMMON);
   if(h != INVALID_HANDLE) {
      string sig = FileReadString(h);
      FileClose(h);
      if(StringFind(sig, "BUY") >= 0) return SIGNAL_BUY;
      if(StringFind(sig, "SELL") >= 0) return SIGNAL_SELL;
   }
   return SIGNAL_NONE;
}
...
//+------------------------------------------------------------------+
void EnviaOrdem(int signal, string reason) {
   int totalSimultaneos = 0;
   for(int i=0; i<PositionsTotal(); i++) {
      if(PositionSelectByTicket(PositionGetTicket(i))) {
         if(PositionGetInteger(POSITION_MAGIC) == EA_MAGIC) totalSimultaneos++;
      }
   }
   if(totalSimultaneos >= p_maxTrades) return;
...
void AddRule(string txt, int intent) {
   ...
   if(StringFind(t, "média") >= 0 || StringFind(t, "media") >= 0) {
      ...
      if(p > 0) lastMA = (int)p;
      r.p1 = lastMA;
      r.handle1 = iMA(_Symbol, (ENUM_TIMEFRAMES)r.tf, 1, 0, MODE_SMA, PRICE_CLOSE); // Price Proxy
      r.handle2 = iMA(_Symbol, (ENUM_TIMEFRAMES)r.tf, r.p1, 0, MODE_EMA, PRICE_CLOSE); // MA
      r.active = true;
   }
      ...
      r.p1 = lastRSI;
      r.d1 = ExtraiNumero(t, cur);
      if(StringFind(t, "acima") >= 0 || StringFind(t, ">") >= 0) r.s1 = ">";
      else if(StringFind(t, "abaixo") >= 0 || StringFind(t, "<") >= 0) r.s1 = "<";
   ...
   else if(StringFind(t, "breakout") >= 0 || StringFind(t, "rompimento") >= 0) {
      r.type = 5;
      r.active = true;
   }
   else if(StringFind(t, "delta") >= 0 || StringFind(t, "agressão") >= 0) {
      r.type = 6;
      int cur = StringFind(t, "delta"); if(cur<0) cur=StringFind(t,"agressão");
      r.p1 = (int)ExtraiNumero(t, cur); if(r.p1==0) r.p1=300;
      r.active = true;
   }
   else if(StringFind(t, "volume") >= 0 || StringFind(t, "ciclo") >= 0) {
      r.type = 7;
      int cur = StringFind(t, "volume");
      r.p1 = (int)ExtraiNumero(t, cur); if(r.p1==0) r.p1=12;
      r.active = true;
   }
   else if(StringFind(t, "ama") >= 0 || StringFind(t, "adaptativa") >= 0) {
      r.type = 8;
      r.p1 = 10;
      r.handle1 = iAMA(_Symbol, (ENUM_TIMEFRAMES)r.tf, r.p1, 2, 30, 0, PRICE_CLOSE);
      r.active = true;
   }
   else if(StringFind(t, "padrão") >= 0 || StringFind(t, "barras") >= 0) {
      r.type = 9;
      r.active = true;
   }
   else if(StringFind(t, "força relativa") >= 0 || StringFind(t, "relativa") >= 0) {
      r.type = 10;
   ...
   if(r.active) {
      if(r.handle1 == INVALID_HANDLE && r.type != 5 && r.type != 6 && r.type != 9) {
         GravaLog("Erro ao criar handle para regra type " + IntegerToString(r.type));
         r.active = false;
      } else {
         g_nRules++;
      }
...
void InterpretaPrompt(string prompt) {
   ...
   for(int i=0; i<n; i++) {
      ...
      if(StringFind(sl, "move stop para entrada") >= 0 || StringFind(sl, "breakeven") >= 0) {
         int curB = StringFind(sl, "atingir");
         if(curB < 0) curB = StringFind(sl, "breakeven");
         if(curB >= 0) {
            curB += 7;
            p_breakeven = ExtraiNumero(sl, curB);
         }
         int curP = StringFind(sl, "entrada +");
         if(curP >= 0) {
            curP += 9;
            p_breakevenPlus = ExtraiNumero(sl, curP);
         }
      }

      if(StringFind(sl, "depois das") >= 0 || StringFind(sl, "após as") >= 0) {
         int curT = StringFind(sl, "das");
         if(curT < 0) curT = StringFind(sl, "as");
         if(curT >= 0) {
            curT += 3;
            while(curT < StringLen(sl) && (StringGetCharacter(sl, curT) == ' ')) curT++;
            p_startTime = StringSubstr(sl, curT, 5);
      ...
      if(StringFind(sl, "trailing") >= 0 || StringFind(sl, "rastreio") >= 0) {
         int curTS = StringFind(sl, "trailing");
         if(curTS < 0) curTS = StringFind(sl, "rastreio");
         curTS += 8;
         p_trailingStop = ExtraiNumero(sl, curTS);
         p_trailingStep = ExtraiNumero(sl, curTS);

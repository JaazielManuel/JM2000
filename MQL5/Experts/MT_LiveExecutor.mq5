...
struct Rule {
   ...
   int        p3_handle;
   bool       is_cross;
   Signal     intent; // Added: directional intent for confluence
};
...
Signal CruzamentoMA(int handle1, int handle2, int shift=1)
{
   ...
   if(handle2 != INVALID_HANDLE) {
      ...
      // Fast cross Slow (handle1 is Fast, handle2 is Slow)
      if(f[1] < s[1] && f[0] > s[0]) return BUY;
      if(f[1] > s[1] && f[0] < s[0]) return SELL;
   } else {
      // Price cross MA (handle1 is MA)
      if(CopyClose(_Symbol, p_frequency, shift, 2, s) <= 0) return NONE;
      // s[0] is current Price, f[0] is current MA
      if(s[1] < f[1] && s[0] > f[0]) return BUY;
      if(s[1] > f[1] && s[0] < f[0]) return SELL;
   }
...
// 1.2 RSI
Signal RSIThreshold(int handle, double over, double under, bool is_cross, Signal intent, int shift=1)
{
   double v[2]; // v[0]=current, v[1]=previous
   if(CopyBuffer(handle, 0, shift, 2, v) <= 0) return NONE;

   if(is_cross) {
      // If momentum intent (buy when rising above), or standard cross
      if(intent == BUY || intent == NONE) {
         if(v[1] <= under && v[0] > under && under > 0) return BUY; // Rising above 'under'
         if(v[1] <= over && v[0] > over && over > 0) return BUY; // Rising above 'over'
      }
      if(intent == SELL || intent == NONE) {
         if(v[1] >= over && v[0] < over && over > 0) return SELL; // Falling below 'over'
         if(v[1] >= under && v[0] < under && under > 0) return SELL; // Falling below 'under'
      }
   } else {
      // Reversion (default if no specific cross keyword)
      if(over > 0 && v[1] > over) return SELL;
      if(under > 0 && v[1] < under) return BUY;
   }
   return NONE;
}
...
Signal BBounce(int handle, int shift=1)
{
   ...
   if(CopyClose(_Symbol, p_frequency, shift, 1, cl) <= 0) return NONE;
...
{
   long vol[];
   if(CopyVolume(_Symbol, p_frequency, shift, len, vol) <= 0) return NONE;
...
Signal Bar2Pattern(int shift=1)
{
   ...
   double c0=iClose(_Symbol, p_frequency, shift);
   double o0=iOpen(_Symbol, p_frequency, shift);
...
{
   int h1 = iRSI(_Symbol, p_frequency, len, PRICE_CLOSE);
   int h2 = iRSI(bench, p_frequency, len, PRICE_CLOSE);
   double v1[1], v2[1];
...
double ExtraiNumero(string txt, string chave) {
   ...
   // Clean up the string to ensure StringToDouble parses correctly
   string result = "";
   for(int i=0; i<StringLen(sub); i++) {
      ushort c = StringGetCharacter(sub, i);
      if((c >= '0' && c <= '9') || c == '.' || c == '-' || c == '+') result += ShortToString(c);
      else if(c == ',') result += ".";
...
int PeriodoTexto(string nome) {
   ...
   if(StringFind(n, "m30") >= 0 || StringFind(n, "30 min") >= 0 || StringFind(n, "30 minutos") >= 0) return PERIOD_M30;
   if(StringFind(n, "m15") >= 0 || StringFind(n, "15 min") >= 0 || StringFind(n, "15 minutos") >= 0) return PERIOD_M15;
   if(StringFind(n, "m5") >= 0  || StringFind(n, "5 min") >= 0  || StringFind(n, "5 minutos") >= 0)  return PERIOD_M5;
   if(StringFind(n, "m1") >= 0  || StringFind(n, "1 min") >= 0  || StringFind(n, "1 minuto") >= 0)   return PERIOD_M1;
...
void ResetStrategy() {
   ...
   nRules = 0;
   lastBarTime = 0;
   p_risk = 1.0; p_stopPoints = 0; p_takePoints = 0;
   p_newsVetoMins = 0; p_maxTrades = 100; p_frequency = PERIOD_CURRENT;
   p_beTrigger = 0; p_bePoints = 0; p_trailingStop = 0;
   p_martingale = false; p_hedge = false; p_notifications = false;
   p_startTime = 0;
}
...
void InterpretaPrompt(string prompt) {
   ...
   // Strategy-wide parameters (pre-parsing)
   int freqMins = (int)ExtraiNumero(prompt, "cada ");
   if(freqMins > 0) p_frequency = (ENUM_TIMEFRAMES)MinutesToTimeframe(freqMins);
   ...
   for(int i=0; i<nParts; i++) {
      ...
      if(StringFind(p, "compra") >= 0) currentIntent = BUY;
      else if(StringFind(p, "vende") >= 0) currentIntent = SELL;

      // Moving Average
      if(StringFind(p, "média") >= 0 || StringFind(p, "ma") >= 0) {
         int per = (int)ExtraiNumero(p, "média de ");
         if(per == 0) per = (int)ExtraiNumero(p, "ma ");
         if(per == 0) {
            // Check for dual period syntax like 9/21 or 200/500
            int slashPos = StringFind(p, "/");
            if(slashPos > 0) {
               // Extract string before and after slash
               int leftStart = slashPos - 1;
               while(leftStart >= 0 && (StringGetCharacter(p, leftStart) >= '0' && StringGetCharacter(p, leftStart) <= '9')) leftStart--;
               string s1 = StringSubstr(p, leftStart + 1, slashPos - leftStart - 1);

               int rightStart = slashPos + 1;
               int rightLen = 0;
               while(rightStart + rightLen < StringLen(p) && (StringGetCharacter(p, rightStart + rightLen) >= '0' && StringGetCharacter(p, rightStart + rightLen) <= '9')) rightLen++;
               string s2 = StringSubstr(p, rightStart, rightLen);

               rules[nRules].p1 = (int)StringToInteger(s1);
               rules[nRules].p2 = (int)StringToInteger(s2);
            } else per = 20;
         }

         rules[nRules].active = true;
         rules[nRules].type = MA_CROSS;
         if(rules[nRules].p1 == 0) rules[nRules].p1 = per;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1_handle = iMA(_Symbol, p_frequency, rules[nRules].p1, 0, MODE_EMA, PRICE_CLOSE);
         if(rules[nRules].p2 > 0)
            rules[nRules].p2_handle = iMA(_Symbol, p_frequency, rules[nRules].p2, 0, MODE_EMA, PRICE_CLOSE);
      ...
      if(StringFind(p, "rsi") >= 0) {
         int per = (int)ExtraiNumero(p, "rsi (");
         if(per == 0) per = (int)ExtraiNumero(p, "rsi ");
         if(per == 0) per = 14;
         ...
         rules[nRules].p1_handle = iRSI(_Symbol, p_frequency, rules[nRules].p1, PRICE_CLOSE);
      ...
      if(StringFind(p, "estocástico") >= 0 || StringFind(p, "stoch") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = STOCH_CROSS;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1_handle = iStochastic(_Symbol, p_frequency, 5, 3, 3, MODE_SMA, STO_LOWHIGH);
         nRules++;
      }
      ...
      if(StringFind(p, "bollinger") >= 0 || StringFind(p, "bb") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = BB_BOUNCE;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1_handle = iBands(_Symbol, p_frequency, 20, 0, 2.0, PRICE_CLOSE);
         nRules++;
      }

      // Daily Breakout
      if(StringFind(p, "rompimento diário") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = DAILY_BREAK;
         rules[nRules].intent = currentIntent;
         nRules++;
      }
      ...
      if(StringFind(p, "delta") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = DELTA_AGG;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1 = 60; // default seconds
         rules[nRules].d1 = 300; // default delta
         nRules++;
      }
      ...
      if(StringFind(p, "volume") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = VOL_CYCLE;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1 = 12; // default len
         nRules++;
      }

      // AMA
      if(StringFind(p, "ama") >= 0 || StringFind(p, "adaptativa") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = AMA_KAUFMAN;
         rules[nRules].intent = currentIntent;
         rules[nRules].p1_handle = iAMA(_Symbol, p_frequency, 10, 2, 30, 0, PRICE_CLOSE);
         nRules++;
      }

      // Bar Pattern
      if(StringFind(p, "padrão barras") >= 0) {
         rules[nRules].active = true;
         rules[nRules].type = BAR_PATTERN;
         rules[nRules].intent = currentIntent;
         nRules++;
      }
   }

   // Strategy-wide parameters (at the end to pick up values after indicator rules)
   string lowerPrompt = prompt; StringToLower(lowerPrompt);
   p_risk = ExtraiNumero(lowerPrompt, "risco de ");
   if(p_risk == 0) p_risk = ExtraiNumero(lowerPrompt, "risco ");
   if(p_risk == 0) p_risk = 1.0;

   p_stopPoints = (int)ExtraiNumero(lowerPrompt, "stop de ");
   if(p_stopPoints == 0) p_stopPoints = (int)ExtraiNumero(lowerPrompt, "stop ");

   p_takePoints = (int)ExtraiNumero(lowerPrompt, "take de ");
   if(p_takePoints == 0) p_takePoints = (int)ExtraiNumero(lowerPrompt, "take ");
   p_newsVetoMins = (int)ExtraiNumero(lowerPrompt, "operar "); // "Não operar 20 min..."
   p_maxTrades = (int)ExtraiNumero(lowerPrompt, "máximo ");
   if(p_maxTrades == 0) p_maxTrades = (int)ExtraiNumero(lowerPrompt, "max ");
   if(p_maxTrades == 0) p_maxTrades = 100;

   int freqMins_val = (int)ExtraiNumero(lowerPrompt, "cada ");
   if(freqMins_val > 0) p_frequency = MinutesToTimeframe(freqMins_val);

   p_beTrigger = (int)ExtraiNumero(lowerPrompt, "atingir +");
   p_bePoints = (int)ExtraiNumero(lowerPrompt, "entrada +");

   if(StringFind(lowerPrompt, "trailing stop") >= 0) p_trailingStop = (int)ExtraiNumero(lowerPrompt, "trailing de ");
   if(StringFind(lowerPrompt, "martingale") >= 0) p_martingale = true;
   if(StringFind(lowerPrompt, "hedge") >= 0) p_hedge = true;
   if(StringFind(lowerPrompt, "notificações") >= 0) p_notifications = true;

   int hour = (int)ExtraiNumero(lowerPrompt, "depois das ");
   if(hour > 0) {
      MqlDateTime dt;
...
Signal AvaliaTudo() {
   ...
   for(int i=0; i<nRules; i++) {
      ...
      switch(rules[i].type) {
         ...
         case RSI_THRESHOLD: s = RSIThreshold(rules[i].p1_handle, rules[i].d1, rules[i].d2, rules[i].is_cross, rules[i].intent); break;
      ...
      if(rules[i].intent == BUY) {
         ...
         buyLegRules++; if(s == BUY) buyLegVotes++;
         sellLegRules++; if(s == SELL) sellLegVotes++;
      }
   }

   if(buyLegRules > 0 && buyLegVotes == buyLegRules) return BUY;
   if(sellLegRules > 0 && sellLegVotes == sellLegRules) return SELL;
...
void OnTimer() {
   ...
   if(GlobalVariableCheck(UPDATE_VAR) && GlobalVariableGet(UPDATE_VAR) != 0) {
      int handle = FileOpen(PROMPT_FILE, FILE_READ | FILE_TXT);
      if(handle != INVALID_HANDLE) {
         string newPrompt = FileReadString(handle);
         FileClose(handle);
         if(StringLen(newPrompt) > 0) {
            InterpretaPrompt(newPrompt);
            Print("Estratégia atualizada via arquivo!");
         }
      }
      GlobalVariableSet(UPDATE_VAR, 0);
   }
}

...
Signal RSIThreshold(int idx, int shift)
{
   ...
   double v = v_buf[0], vp = v_buf[1]; // Corrected index for newest values (buf[0] is newest in CopyBuffer if not AS_SERIES)
...
Signal DailyBreak(int idx, int shift)
{
   ...
   double hi = rates[1].high, lo = rates[1].low; // Previous day
...
Signal AMA(int idx, int shift)
{
   ...
   if(buf[0] > buf[1]) return BUY; // buf[0] is newest
...
double ExtraiNumero(string txt, string prefix)
{
   ...
   // Find the end of the number (first non-numeric, non-dot, non-comma character)
   int end = 0;
   while(end < StringLen(sub))
   {
      short c = StringGetCharacter(sub, end);
      if((c < '0' || c > '9') && c != '.' && c != ',') break;
      end++;
   }
   string num_str = StringSubstr(sub, 0, end);
...
void InterpretaPrompt(string prompt)
{
   ...
   // Parse Risk
   if(StringFind(p, "risco de") >= 0) g_risk = ExtraiNumero(p, "risco de ");
   else if(StringFind(p, "risco") >= 0) g_risk = ExtraiNumero(p, "risco ");

   // Parse Stop/Take
   if(StringFind(p, "stop de") >= 0) g_stop_pts = (int)ExtraiNumero(p, "stop de ");
   else if(StringFind(p, "stop") >= 0) g_stop_pts = (int)ExtraiNumero(p, "stop ");

   if(StringFind(p, "take de") >= 0) g_take_pts = (int)ExtraiNumero(p, "take de ");
   else if(StringFind(p, "take") >= 0) g_take_pts = (int)ExtraiNumero(p, "take ");
   ...
   {
      g_be_trigger = (int)ExtraiNumero(p, "atingir +");
      g_be_plus = (int)ExtraiNumero(p, "entrada +");
   }
   ...
   if(StringFind(p, "rsi") >= 0)
   {
      ...
      rules[nRules].d1 = ExtraiNumero(p, "subir acima de ");
      if(rules[nRules].d1 == 0) rules[nRules].d1 = 55;

      rules[nRules].d2 = ExtraiNumero(p, "cair abaixo de ");
   ...
   Print("Prompt interpretado: " + prompt);
}
...
void EnviaOrdem(Signal s, double lote, int sl_pts, int tp_pts)
{
   ...
   if(res) Print("Ordem enviada com sucesso: " + EnumToString(s));
   else Print("Erro ao enviar ordem: " + IntegerToString(g_trade.ResultRetcode()));
}
...
void AIOptimizer()
{
   // Heuristic: Suggest Risk/SL adjustments based on 30-day Win Rate and ATR
   ...
   {
      g_risk = MathMax(0.5, g_risk * 0.9);
      GravaLog("AI Heuristic: Win rate low (" + DoubleToString(winRate,2) + "). Reducing risk to " + DoubleToString(g_risk,2));
   }
}

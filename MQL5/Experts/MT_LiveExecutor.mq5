...
const int MAX_BURST = 5;

// Cached Symbol Properties
double cacheTickSize = 0;
double cacheVolStep = 0;
double cacheVolMin = 0;
double cacheVolMax = 0;
int    cacheStopsLevel = 0;

// High-performance normalization
double NS(double price) {
    if(cacheTickSize <= 0) return price;
    return NormalizeDouble(MathRound(price / cacheTickSize) * cacheTickSize, _Digits);
}

double NV(double volume) {
    if(cacheVolStep <= 0) return volume;
    double vol = NormalizeDouble(MathFloor(volume / cacheVolStep) * cacheVolStep, 2);
    if (vol < cacheVolMin) vol = cacheVolMin;
    if (vol > cacheVolMax) vol = cacheVolMax;
    return vol;
}
...
bool CheckThrottling() {
    ...
    if (tradeBurstCount >= MAX_BURST) {
        Print("MT-LiveExecutor: Throttling ativo. Limite de burst atingido.");
        return false;
    }
    tradeBurstCount++;
    return true;
...
ENUM_TIMEFRAMES PeriodoTexto(string nome) {
    StringToLower(nome);
    // Ordenado do maior para o menor para evitar problemas de substring (ex: "m15" contém "m1")
...
void InterpretaPrompt(string prompt) {
    ...
    if (pos >= 0) {
        ...
        if (pAbove >= 0 && pAbove < pos + 40) {
            int offset = (StringFind(prompt, "superior a ", pos) >= 0) ? 11 : 9;
            r.d1 = GetNextNumber(prompt, pAbove + offset);
        }

        int pBelow = StringFind(prompt, "abaixo de ", pos);
        if (pBelow < 0) pBelow = StringFind(prompt, "inferior a ", pos);
        if (pBelow >= 0 && pBelow < pos + 40) {
            int offset = (StringFind(prompt, "inferior a ", pos) >= 0) ? 11 : 10;
            r.d2 = GetNextNumber(prompt, pBelow + offset);
        }
...
datetime lastErrorCooldownTime = 0;

void HandleTradeError(string action) {
    uint retCode = trade.ResultRetcode();
    int lastError = GetLastError();
    string msg = StringFormat("ERRO %s: RetCode=%u (%s), LastError=%d", action, retCode, trade.ResultComment(), lastError);
    GravaLog(msg);

    // Dynamic safety adjustment
    if (retCode == TRADE_RETCODE_INVALID_STOPS || retCode == 10017 /*TRADE_RETCODE_FROZEN*/) {
        dynamicSafetyPoints = MathMin(dynamicSafetyPoints + 2, 100);
        PrintFormat("MT-LiveExecutor: Ajustando dynamicSafetyPoints para %d devido a erro de execução. Cooldown de 2s ativado.", dynamicSafetyPoints);
        lastErrorCooldownTime = TimeCurrent();
    }
}
...
int CountPositions() {
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if (posInfo.SelectByTicket(ticket)) {
            if (posInfo.Magic() == 123456 && posInfo.Symbol() == _Symbol) count++;
        }
    }
    return count;
}
...
bool IsPriceSafe(double price, ENUM_SIGNAL side, bool isSL = false) {
    double minDistance = (cacheStopsLevel + dynamicSafetyPoints + 1) * _Point;
...
ENUM_SIGNAL AvaliaCondicoes() {
    ...
    if (CountPositions() >= currentStrategy.maxTrades) return SIGNAL_NONE;
...
void GerenciaPosicoes() {
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if (posInfo.SelectByTicket(ticket)) {
            ...
            // Breakeven logic (with modification noise reduction)
            if (currentStrategy.breakevenTriggerPoints > 0) {
                if (posInfo.PositionType() == POSITION_TYPE_BUY) {
                    if (currentPrice >= openPrice + currentStrategy.breakevenTriggerPoints * _Point) {
                        ...
                        if (newSL > stopLoss + _Point && CheckThrottling()) trade.PositionModify(posInfo.Ticket(), newSL, NS(takeProfit));
                    }
                } else if (posInfo.PositionType() == POSITION_TYPE_SELL) {
                    if (currentPrice <= openPrice - currentStrategy.breakevenTriggerPoints * _Point) {
                        double newSL = NS(openPrice - currentStrategy.breakevenProfitPoints * _Point);
                        if ((stopLoss == 0 || newSL < stopLoss - _Point) && CheckThrottling()) trade.PositionModify(posInfo.Ticket(), newSL, NS(takeProfit));
                    }
                }
            }

            // Trailing Stop logic
            if (currentStrategy.trailingStopPoints > 0 && CheckThrottling()) {
                if (posInfo.PositionType() == POSITION_TYPE_BUY) {
                    if (currentPrice > openPrice + currentStrategy.trailingStopPoints * _Point) {
                        ...
                        if (newSL > stopLoss + currentStrategy.trailingStepPoints * _Point) trade.PositionModify(posInfo.Ticket(), newSL, NS(takeProfit));
                    }
                } else if (posInfo.PositionType() == POSITION_TYPE_SELL) {
                    if (currentPrice < openPrice - currentStrategy.trailingStopPoints * _Point) {
                        double newSL = NS(currentPrice + currentStrategy.trailingStopPoints * _Point);
                        if ((newSL < stopLoss - currentStrategy.trailingStepPoints * _Point) || stopLoss == 0) trade.PositionModify(posInfo.Ticket(), newSL, NS(takeProfit));
                    }
                }
            }
        }
...
void UpdateSymbolCache() {
    cacheTickSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
    cacheVolStep  = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
    cacheVolMin   = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
    cacheVolMax   = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
    cacheStopsLevel = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
}
...
void OnTick() {
    ...
    if (isNewBar) {
        if (TimeCurrent() < lastErrorCooldownTime + 2) return;

        ENUM_SIGNAL sig = AvaliaCondicoes();
        if (sig != SIGNAL_NONE && CheckThrottling()) {
            ...
            if (sig == SIGNAL_BUY) {
                if (currentStrategy.stopLossPoints > 0) {
                    sl = NS(price - currentStrategy.stopLossPoints * _Point);
                    if (!IsPriceSafe(sl, SIGNAL_BUY, true)) sl = NS(currentTick.bid - (symbolInfo.StopsLevel() + dynamicSafetyPoints + 1) * _Point);
                }
                if (currentStrategy.takeProfitPoints > 0) tp = NS(price + currentStrategy.takeProfitPoints * _Point);

                if (trade.Buy(lot, _Symbol, NS(price), sl, tp, "MT-LiveExecutor BUY"))
                ...
                if (currentStrategy.stopLossPoints > 0) {
                    sl = NS(price + currentStrategy.stopLossPoints * _Point);
                    if (!IsPriceSafe(sl, SIGNAL_SELL, true)) sl = NS(currentTick.ask + (symbolInfo.StopsLevel() + dynamicSafetyPoints + 1) * _Point);
                }
                if (currentStrategy.takeProfitPoints > 0) tp = NS(price - currentStrategy.takeProfitPoints * _Point);

                if (trade.Sell(lot, _Symbol, NS(price), sl, tp, "MT-LiveExecutor SELL"))

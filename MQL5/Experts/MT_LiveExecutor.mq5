...
void InterpretaPrompt(string prompt) {
   ...
   if(StringFind(prompt, "stop de") >= 0) {
      int val = ExtractInt(prompt, "stop de");
      if(val > 0) settings.stop_pts = val * 10; // converte pips p/ pontos se necessário, ou mantém se for pontos
      if(StringFind(prompt, "pontos") >= 0 && StringFind(prompt, "stop de") < StringFind(prompt, "pontos")) settings.stop_pts = val;
   }
   if(StringFind(prompt, "take de") >= 0) {
      int val = ExtractInt(prompt, "take de");
      if(val > 0) settings.take_pts = val;
   }
   if(StringFind(prompt, "risco de") >= 0) {
      settings.risk_perc = ExtractDouble(prompt, "risco de");
   }
   if(StringFind(prompt, "máximo") >= 0 && StringFind(prompt, "trades") >= 0) {
   ...
   if(StringFind(prompt, "a cada") >= 0 && StringFind(prompt, "minutos") >= 0) {
   ...
   if(StringFind(prompt, "média") >= 0 && StringFind(prompt, "rsi") >= 0) {
      int pMA = ExtractInt(prompt, "média de"); if(pMA==0) pMA=20;
      ...
      rules[nRules].tf = tf;
      rules[nRules].desc = "Cruzamento MA";
      nRules++;
   }

   if(StringFind(prompt, "estocástico") >= 0) {
      rules[nRules].active = true;
...
void OnTick() {
   ...
   if(PositionsTotal() < settings.max_trades) {
...
bool IsNewsVetoed() {
   // FILTRO DE NOTÍCIAS (MQL5 Nativo):
   // 1. Usar CalendarValueHistory ou CalendarValueLast
   // 2. MqlCalendarValue valores[];
   // 3. if(CalendarValueLast(change_id, valores) > 0) ...
   // Por padrão, mantemos desativado para evitar bloqueios sem dados.
   return false;
}
...
void AIOptimizer() {
   // Analisa os últimos trades e sugere ajustes de SL/TP
   Print("AI Module: Analisando performance para otimização...");
}
...
ENUM_SIGNAL RSIThreshold(int h1, int h2, int p1, int p2, double d1, double d2, uint tf, int shift) {
   double r[1];
   if(CopyBuffer(h1, 0, shift, 1, r) < 1) return SIGNAL_NONE;
   if(r[0] > d1) return SIGNAL_SELL;
   if(r[0] < d2) return SIGNAL_BUY;
   return SIGNAL_NONE;
}

ENUM_SIGNAL MACrossRSITrend(int h1, int h2, int p1, int p2, double d1, double d2, uint tf, int shift) {
   double ma[1], rsi[1], close[1];
   if(CopyBuffer(h1, 0, shift, 1, ma) < 1 || CopyBuffer(h2, 0, shift, 1, rsi) < 1) return SIGNAL_NONE;
   CopyClose(_Symbol, (ENUM_TIMEFRAMES)tf, shift, 1, close);
   if(close[0] > ma[0] && rsi[0] > d1) return SIGNAL_BUY;
   if(close[0] < ma[0] && rsi[0] < d2) return SIGNAL_SELL;
   return SIGNAL_NONE;
}
...
double CalculaLote(double riscoPerc) {
   ...
   double lot = riskMoney / (settings.stop_pts * (tickValue / tickSize));
...
void EnviaOrdem(ENUM_SIGNAL tipo, double lote) {
   ...
   if(res) {
      GravaLog("Ordem Enviada: " + EnumToString(tipo) + " Lote: " + DoubleToString(lote, 2));
   } else {

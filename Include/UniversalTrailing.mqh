...
/*
   SISTEMA DE TRAILING STOP UNIVERSAL INTELIGENTE (MYTHICAL MASTER V6.1 - 2026)

   O nível máximo de engenharia MQL5:
   - Ultra-Precise Throttling: Microsecond timing (GetMicrosecondCount) e overflow-proof.
   - Zero-Rejection Engine: Cache de Stop/Freeze levels com margem de segurança cirúrgica.
   - Legendary Handle Safety: BarsCalculated() rigoroso para evitar sinais falsos/vazios.
   - Institutional Volatility scaling: Dual-ATR paramétrico (Fator de Escala 2.0 - 10.0).
   - Zero Dynamic Allocation: Buffers HL pré-alocados e cache de propriedades de símbolo.
   - Diagnostic Master: Logging inteligente de modificações e falhas de inicialização.
*/
...
private:
   ...
   bool           m_fractal_failed; // Cache de falha de criação
...
CUniversalTrailing::CUniversalTrailing() :
   ...
   m_fractal_failed(false),
   m_max_spread(0),
   m_throttle_ms(250),
   m_last_tick_us(0),
   m_last_level_refresh(0),
...
void CUniversalTrailing::Init(long magic, string symbol_name)
{
   ...
   RefreshSymbolLevels();
   Print("CUniversalTrailing initialized for Symbol: ", m_symbol_name, " Magic: ", m_magic);
}
...
void CUniversalTrailing::CheckHandle(int handle, string name)
{
   if(handle == INVALID_HANDLE)
      Print("CRITICAL ERROR [", m_symbol_name, "]: Failed to create ", name, " handle. Last Error: ", GetLastError());
}
...
void CUniversalTrailing::SetHL(int candles)
{
   ...
   if(ArraySize(m_hl_buffer) < m_hl_candles) ArrayResize(m_hl_buffer, m_hl_candles + 20);
}
...
void CUniversalTrailing::SetFractals()
{
   ...
   if(m_fractal_handle == INVALID_HANDLE) m_fractal_failed = true;
...
//+------------------------------------------------------------------+
//| Main Process Loop (Surgical Master v6.1)                         |
//+------------------------------------------------------------------+
void CUniversalTrailing::Process()
{
   // 1. Precise Throttling
   ...
   {
      static uint refresh_fail_count = 0;
      if(refresh_fail_count++ % 100 == 0) Print("Warning: m_symbol.RefreshRates() failed for ", m_symbol_name);
      return;
   }
   ...
   // Refresh Symbol Levels every 10 seconds
   if(TimeCurrent() - m_last_level_refresh > 10) RefreshSymbolLevels();
   ...
   // 4. Tick Caching (Institutional HFT)
   double cached_atr = 0, cached_atr_slow = 0, cached_psar = 0, cached_ma = 0;
   if(m_mode == TRL_MODE_ATR) {
      cached_atr = GetIndicatorValue(m_atr_handle, 1, "ATR Fast");
      cached_atr_slow = GetIndicatorValue(m_atr_handle_slow, 1, "ATR Structural");
   }
   else if(m_mode == TRL_MODE_PSAR) cached_psar = GetIndicatorValue(m_psar_handle, 1, "PSAR");
   else if(m_mode == TRL_MODE_MA)   cached_ma = GetIndicatorValue(m_ma_handle, 1, "MA");
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(m_position.SelectByIndex(i))
      {
         ...
         // Early Exit: Trailing Estrutural
         ...
         if(m_be_activation > 0)
         {
            ...
            if(profit_pts >= m_be_activation)
            {
               ...
               {
                  if(ModifySL(m_position.Ticket(), be_price, current_tp, "Breakeven"))
                     current_sl = be_price;
               }
            }
         }
         ...
         // C. Directional Integrity & Safety Guard
         if(new_sl > 0)
         {
            ...
            if(new_sl > 0)
            {
               ...
               {
                  // Explicit semantics for current_sl == 0
               ...
               {
                  ModifySL(m_position.Ticket(), new_sl, current_tp, "Trailing Master");
               }
            }
         }
      }
...
double CUniversalTrailing::GetIndicatorValue(int handle, int index, string name)
{
   ...
   if(BarsCalculated(handle) < index + 1)
   {
      static datetime last_warn = 0;
      if(TimeCurrent() - last_warn > 30) { Print("Warning: ", name, " data not ready for ", m_symbol_name); last_warn = TimeCurrent(); }
      return 0;
   }
...
double CUniversalTrailing::GetBollingerValue(ENUM_POSITION_TYPE type, int index)
{
   ...
   double buffer[1];
   int buffer_idx = (type == POSITION_TYPE_BUY) ? 2 : 1;
   if(CopyBuffer(m_bb_handle, buffer_idx, index, 1, buffer) < 1) return 0;
   return buffer[0];
}

//+------------------------------------------------------------------+
//| Get High/Low Value (Zero Allocation)                             |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Get Fractal Value (Surgical)                                     |
//+------------------------------------------------------------------+
double CUniversalTrailing::GetFractalValue(ENUM_POSITION_TYPE type, int index)
{
   if(m_fractal_handle == INVALID_HANDLE && !m_fractal_failed) SetFractals();
   if(m_fractal_handle == INVALID_HANDLE) return 0;
...
{
   double arr[1];
   if(type == POSITION_TYPE_BUY) { if(CopyLow(m_symbol_name, PERIOD_CURRENT, index, 1, arr) > 0) return arr[0]; }
   else { if(CopyHigh(m_symbol_name, PERIOD_CURRENT, index, 1, arr) > 0) return arr[0]; }
   return 0;
}
...
//+------------------------------------------------------------------+
bool CUniversalTrailing::ModifySL(long ticket, double new_sl, double current_tp, string reason)
{
   ...
   Print("✓ [", m_symbol_name, "] SL Modificado (#", ticket, ") via ", reason, " para ", new_sl);
   return true;
}
